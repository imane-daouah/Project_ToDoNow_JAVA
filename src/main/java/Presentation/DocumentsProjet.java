...
import javax.swing.*;
import java.awt.*;
public class DocumentsProjet extends JFrame {

    private static final long serialVersionUID = 1L;

    public DocumentsProjet() {
    ...
    private void initializeUI() {
        ...
        JButton addButton = new JButton("Creé Un Document");
        addButton.setBackground(new Color(154, 215, 237));
        addButton.addActionListener(e -> ajouterDocument());
        navPanel.add(addButton);
        
        JButton impButton = new JButton("Importer Un Document");
        impButton.setBackground(new Color(154, 215, 237));
        impButton.addActionListener(e -> ImporterDocument());
        navPanel.add(impButton);
    ...
    private void ajouterDocument() {
		dispose();
        CreéDocument creer = new CreéDocument();
        creer.setVisible(true);
    }
	private void ImporterDocument() {
		dispose();
		importerDocument creer = new importerDocument();
        creer.setVisible(true);
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            new DocumentsProjet().setVisible(true);
        });
    }
}
